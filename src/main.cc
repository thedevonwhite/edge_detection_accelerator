...
static const int AXIDMA_ID          = XPAR_AXI_DMA_DEVICE_ID;
...
static int edge_detection(axidma_t axidma, const TCHAR *image_dir,
...
static int run_edge_detections(axidma_t& axidma, const TCHAR* image_dir_path,
        DIR *image_dir, const TCHAR *output_dir_path)
{
    ...
    while (true) {
        ...
        int rc = edge_detection(axidma, image_dir_path, file_info.fname,
                output_dir_path);
        if (rc != XST_SUCCESS) {
            return rc;
        }
...
int main()
{
    ...
    rc = run_edge_detections(devices_context.axidma, IMAGE_DIR_PATH, &input_dir,
            OUTPUT_DIR_PATH);
    if (rc != XST_SUCCESS) {
        return rc;
    }

    printf("\nEdge detection is complete. Output images can be found in %s.\n",
            OUTPUT_DIR_PATH);
    return XST_SUCCESS;
}
